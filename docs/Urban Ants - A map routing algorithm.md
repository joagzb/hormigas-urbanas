...
$$\eta_{ij} = \frac{1}{\tilde{d}_{ij}}$$

where $\tilde{d}_{ij}$ is the selection-only surrogate and the reported route cost remains the sum of original $d_{ij}$ values.
...
- Lower score $\rightarrow$ more pheromone
- Higher score $\rightarrow$ less pheromone
...
Once all $m$ ants complete their paths, graph-wide evaporation occurs across all edges $(i, j) \in E$:

$$\tau_{ij} = (1 - \rho)\tau_{ij} + \sum_{k} \Delta\tau_{ij}^k$$

Where:

- $\tau_{ij}$ = pheromone on edge between nodes $i$ and $j$
- $\rho \in [0,1]$ $\rightarrow$ global evaporation rate
- $\Delta\tau_{ij}^k$ $\rightarrow$ pheromone deposited by ant $k$ at edge $(i, j)$
...
- $\alpha$ = pheromone importance. Controls the influence of accumulated historical pheromone trails.
- $\beta$ = heuristic importance. Controls the influence of local heuristic desirability.

### **Heuristic Information**

$$\eta_{ij} = \frac{1}{\tilde{d}_{ij}}$$

- $\tilde{d}_{ij}$ = selection-only edge weight; route cost still uses the original $d_{ij}$
...
Global update is applied only to the edges belonging to the global-best path $T^{\text{gb}}$:

$$\tau_{ij} \leftarrow (1 - \rho) \tau_{ij} + \rho \Delta \tau_{ij}^{\text{gb}} \quad \forall (i,j) \in T^{\text{gb}}$$

$$\Delta \tau_{ij}^{\text{gb}} = \frac{1}{L_{\text{gb}}}$$
...
**Positive Reinforcement**: Pheromone is added to the global-best tour $T^{\text{gb}}$:

$$\tau_{ij} \leftarrow \tau_{ij} + \Delta \tau_{ij}^{\text{gb}} \quad \forall (i,j) \in T^{\text{gb}}$$

**Negative Reinforcement (Worst-Path Penalization)**: For the current iteration's worst tour $T^{\text{worst}}$, any edge $(i, j) \in T^{\text{worst}}$ that is not part of $T^{\text{gb}}$ receives an extra evaporation penalty:

$$\tau_{ij} \leftarrow (1 - \rho_w) \tau_{ij} \quad \forall (i,j) \in T^{\text{worst}} \setminus T^{\text{gb}}$$

### Dynamic Pheromone Mutation

To perturb stagnant search trails, each matrix row $i$ undergoes stochastic mutation with probability $P_{\text{mut}}$:

$$\tau_{ij}' = \max(\tau_{\text{min}}, \tau_{ij} \pm s \cdot \sigma \cdot \bar{\tau}_{\text{gb}})$$

where:

- $z \in \{0, 1\}$ is a uniform binary random variable.
- $\bar{\tau}_{\text{gb}}$ is the average pheromone intensity across edges of $T^{\text{gb}}$.
- $s$ is the configured mutation scale, and $\sigma$ grows with search progress since the last restart:

$$\sigma = \frac{i_{\text{current}} - i_{\text{restart}}}{i_{\text{max}}}$$

## Hard Restart Mechanism

If $T^{\text{gb}}$ fails to improve for 8 iterations, all matrix entries are reset to $\tau_0$, clearing accumulated bias while preserving the best score found. This pheromone-diversity restart is separate from, and does not reset, the shared global-best patience termination policy.
