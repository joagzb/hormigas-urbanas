...
def validate_graph(graph, require_edge_types=True):
    ...
    for node in nodes:
        ...
        if node_edge_types is not None and any(
            edge_type not in EDGE_TYPES for edge_type in node_edge_types
        ):
...
def merge_bus_and_map_graph(map_graph, buses_graph):
    ...
    has_tagged_bus_nodes = any(
        isinstance(node, str) and node.startswith("bus:")
        for node in merged_graph.get("node_index", [])
    )
    if (
        "edge_types" not in merged_graph
        and not merged_graph.get("buses")
        and not has_tagged_bus_nodes
    ):
        merged_graph["edge_types"] = {
            node: ["walk"] * len(connections)
            for node, connections in merged_graph["connections"].items()
        }
...
def deterministic_route_cost(graph_map, start_node, end_node):
    ...
    def ordered_edges(node):
        ...
        return iter(
            sorted(
                zip(node_connections, node_weights),
