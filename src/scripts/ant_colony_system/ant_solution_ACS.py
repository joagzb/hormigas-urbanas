...
def ant_solution_ACS(
    ...
    while solution_path[-1] != end_node:
        ...
        neighbors_edge_types = np.array(
            graph_map.get("edge_types", {}).get(current_node, ["walk"] * len(neighbors)),
            dtype=object,
        )
        ...
        q = np.random.rand()
        if q <= q0:
            desirability = (neighbors_pheromones ** alpha) * (
                (1.0 / selection_weights) ** beta
            )
        ...
        if tau0 is not None and local_evap_rate:
            ...
            pheromone_graph[current_node][edge_index] = (
                (1 - local_evap_rate) * current_pheromone
                + local_evap_rate * tau0
            )
