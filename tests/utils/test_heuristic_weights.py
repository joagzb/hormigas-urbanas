...
from src.scripts.utils.heuristic_weights import normalize_for_selection
...
def test_normalization_uses_edge_semantics_not_matching_numeric_costs():
  ...
  adjusted = normalize_for_selection(weights, edge_types)
...
def test_zero_cost_alighting_gets_positive_selection_only_weight():
  ...
  adjusted = normalize_for_selection(weights, np.array(['alight'], dtype=object))
