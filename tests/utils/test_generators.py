...
def test_merge_bus_and_map_graph_adds_aligned_transfer_edges(monkeypatch):
    ...
    assert all(
        len(merged["connections"][node]) == len(merged["weights"][node]) == len(merged["edge_types"][node])
        for node in merged["node_index"]
    )
...
def test_transfer_route_contains_two_board_edges():
    ...
    path_edge_types = [
        merged["edge_types"][start][merged["connections"][start].index(end)]
...
def test_deterministic_route_cost_backtracks_from_cycles_and_uses_aligned_weights():
    graph = {
        "node_index": {0, 1, 2, 3},
        "connections": {0: [2, 1], 1: [0], 2: [3], 3: []},
        "weights": {0: [4.0, 1.0], 1: [1.0], 2: [2.0], 3: []},
    }
