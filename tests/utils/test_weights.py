...
from src.scripts.utils.weights import get_connection_weight, haversine
...
def test_missing_edge_returns_none():
  graph = {'connections': {0: [1]}, 'weights': {0: [1.0]}}
...
def test_get_connection_weight():
  graph = {'connections': {0: [1], 1: []}, 'weights': {0: [5.0], 1: []}}
  start_node, end_node = 0, 1
  logger.info('Testing get_connection_weight with graph=%s, start_node=%s, end_node=%s', graph, start_node, end_node)
...
def test_haversine():
  lat1, lon1 = 0.0, 0.0
  lat2, lon2 = 0.0, 1.0
  logger.info('Testing haversine with lat1=%s, lon1=%s, lat2=%s, lon2=%s', lat1, lon1, lat2, lon2)
